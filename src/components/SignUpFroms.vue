...
export default {
    ...
    methods: {
        ...
        handelSubmit(){
            // console.log('email',this.email, 'password',this.password,'role', this.role,'trems', this.terms, 'name', this.name,'skills', this.skills)
            // Email Validation is Required
            this.nameError = this.name.includes(' ') ? '' : 'Enter Your Full Name'

            // Email Validation is Required
            this.emailError = this.email.includes('@') && this.email.includes('.com') ? '' : 'Email Must Be Valid'

            // Password Validation is Required
            this.passwordError = this.password.length > 6 ? '' : 'Password Must Be 6 Character'

            // Role Validation is Required
            this.roleError = this.role === null ? 'Role Must Be Selected' : ''
